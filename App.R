...
## Purpose of script:
##
## Author: Fidel Lozano-Elena, Veredas Coleto Alcudia
##
...
source("modules/module_upload_dataset.R")
source("modules/module_dataset_selector.R")
...
ui <- dashboardPage(
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      #=== Upload or use availabale data reactive tab
      tabItem(tabName = "dynamicDataset",
              
              uiOutput("dynamicdatasetUI")
...
server <- function(input, output, session) {
  ...
  #== NEW SEARCH TAB ====
  ## button for list or comparisons in multiple dataset analysis
  input_data <- reactiveValues(submit_own = NULL,
                               submit_upload = NULL)
  
  observeEvent(input$tabs, {
    if (input$tabs == "new_search") {
      # Execute module experiment selector (every time back in new search page)
      # Select how to input the data
      data <- dataset_selectorServer("x", input_data=input_data)
      
      
    }
  })
  
  # PRESSING use our own data BUTTON IN input DATASETS ====
  observeEvent(input_data$submit_own,{
    ...
    output$dynamic_tabs <- renderMenu({
      sidebarMenu(
        ...
        menuItem(
          ...
          tabName = "dynamicDataset",
          icon = icon("list-check", lib = "font-awesome")
        )
      )
    })
    
    # Move to the dynamic tab
    updateTabItems(session = session,
                   inputId = "tabs",
                   selected = "dynamicDataset")
    
    # EXECUTE experiment selector
    ...
    output$dynamicdatasetUI <- renderUI({
      experiment_selectorUI("x")
    })
  })
  
  # PRESSING upload your data BUTTON IN input DATASETS ====
  observeEvent(input_data$submit_upload,{
    ...
    # Move to the dynamictab tab
    updateTabItems(session = session,
                   inputId = "tabs",
                   selected = "dynamicDataset")
    
    # EXECUTE upload dataset
    x <<- upload_datasetServer("upload")
    
    single_cell_experiment <<- FALSE
    
    output$dynamicdatasetUI <- renderUI({
      upload_datasetUI("upload")
    })
  })
  ...
  observeEvent(input$submit,{
    ...
    if(is.null(input_data$submit_upload)==F){ ## For using user dataset
